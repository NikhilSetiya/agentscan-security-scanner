// Supabase Edge Function to store secrets
// This function provides secure storage for application secrets
...
serve(async (req) => {
  ...
  try {
    ...
    if (!/^[A-Z_][A-Z0-9_]*$/.test(body.name)) {
      return new Response(
        JSON.stringify({ error: 'Invalid secret name format' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    // Store secret in Supabase secrets table (upsert)
