...
import { CodeActionsProvider, FindingHoverProvider } from './providers/CodeActionsProvider';
...
let hoverProvider: FindingHoverProvider;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        cacheManager = new CacheManager(context, config.get<number>('cacheMaxAge', 300));
        telemetryService = new TelemetryService(context, config.get<boolean>('enableTelemetry', true));
        navigationService = new NavigationService(diagnosticsManager);
        
        // Initialize providers
        agentScanProvider = new AgentScanProvider(apiClient, diagnosticsManager, config, cacheManager, telemetryService);
        findingsProvider = new FindingsProvider(apiClient);
        codeActionsProvider = new CodeActionsProvider(diagnosticsManager, telemetryService);
        hoverProvider = new FindingHoverProvider(diagnosticsManager, telemetryService);
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        learnMoreCommand,
        nextFindingCommand,
        previousFindingCommand,
        toggleRealTimeScanningCommand,
        showSecurityHealthCommand
    );
}
    ...
    const onDidChangeConfiguration = vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('agentscan')) {
            ...
            if (event.affectsConfiguration('agentscan.enableRealTimeScanning')) {
                telemetryService.trackConfigurationChange(
                    'enableRealTimeScanning',
                    !config.isRealTimeScanningEnabled(),
                    config.isRealTimeScanningEnabled()
                );
            }
            ...
            cacheManager.updateMaxAge(config.get<number>('cacheMaxAge', 300));
            telemetryService.setEnabled(config.get<boolean>('enableTelemetry', true));
...
async function showLearnMore(finding: any) {
    if (!finding) {
...
function createLearnMoreContent(finding: any): string {
    ...
    <body>
        ...
        <div class="section">
            <h3>References</h3>
            ${finding.references.map((ref: string) => 
...
function createSecurityHealthContent(
    findingStats: any, 
    cacheStats: any, 
    connectionStats: any
): string {
...
function showWelcomeMessage(context: vscode.ExtensionContext) {
    ...
    if (!hasShownWelcome) {
        vscode.window.showInformationMessage(
            'Welcome to AgentScan Security! Configure your API key in settings to get started.',
            'Open Settings',
            'Learn More'
        ).then(selection => {
